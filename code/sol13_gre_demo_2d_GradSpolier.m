%% Task: 
% At the end of each TR, there might be some residual transverse magnetization
% Add gradient spoiling to kill it


%% 0. Pre settings
clear; close all; clc;
root = "E:\UIHBranch\BRANCHES\uMRV10_3T_R3\UIH\appdata\MRSiteData\Share\Imaging\pulseq\";
seq_name = "ex13_gre_demo_2d_GradSpolier";
userID = '_lz';
...
gxSpoil = mr.makeTrapezoid('x', 'Area', 2*gx.area, 'system', sys) ;
...
assert(delayTE >= 0);
assert(delayTR >= mr.calcDuration(gxSpoil));
...
for i=1:Ny
    ...
    seq.addBlock(mr.makeDelay(delayTE));
    seq.addBlock(gx, adc);
    seq.addBlock(mr.makeDelay(delayTR), gxSpoil);
end
